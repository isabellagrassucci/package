...
#' @param res DESeq2 results table. Accepts both a file (.tsv, .csv, tab-separated .txt) or a dataframe (see example below). Requires 'log2FoldChange' and 'padj' columns.
#' @param my_comparison The comparison to plot (control_vs_treatment, a_vs_b, ...).
...
volcanoplot <- function (res, my_comparison = NULL, highlight_genes = NULL, log2FC_thresh = 0, padj_thresh = 0.05, del_csv = ",", where_results = "./", outfolder = "results/") {
  ...
  if (is.data.frame(res)){
    ...
    res_path <- res
    res <- read_tsv(res, col_types = cols())
  } else if (grepl(".csv", res)) {
    res_path <- res
    res <- read_delim(res, col_types = cols(), delim = del_csv)
  } else if (grepl(".txt")) {
    res_path <- res
